...
def load_orders_data() -> List[Dict]:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Не удалось создать резервную копия orders_data.json: {ex}")
...
def save_order_data(
    ...
    logger.info(f"Данные заказа #{order_id} (ID заказа: {service_order_id}) сохранены.")
...
def check_order_status(
    ...
    logger.info(f"{LOGGER_PREFIX} Проверка статуса заказа #{service_order_id}, попытка {attempt}...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Ответ сервиса #{service_order_id}: {data_}")
            ...
            # Интеллектуальный анализ статуса
            status_lower = str(status_).lower()
            
            # Определение статусов завершения
            completed_statuses = [
                "complete", "completed", "done", "success", "finished", "завершен", "выполнен"
            ]
            
            # Определение статусов ошибок
            failed_statuses = [
                "fail", "failed", "error", "cancel", "cancelled", "отменен", "ошибка"
            ]
            ...
            if is_completed or (remains_ is not None and remains_ == 0):
                if order_info:
                    try:
                        ...
                        logger.error(f"Ошибка при сохранении статус уведомления для заказа #{order_id_funpay}: {e}")

                order_link = f"https://funpay.com/orders/{order_id_funpay}/"
                message = (
                    f"🎉 Ваш заказ успешно завершён!\n"
                    f"🔢 Номер заказа: {service_order_id}\n"
                ...
                logger.info(
                    f"Уведомление о завершении отправлено покупателю {buyer_chat_id} (заказ #{service_order_id}).")
                ...
                logger.info(f"Заказ #{service_order_id} в статусе '{status_}' (осталось: {remains_}).")
                delay = 300

        elif response.status_code == 429:
            logger.warning(f"Получен статус 429 (Too Many Requests) для заказа #{service_order_id}.")
            delay = 3600

        else:
            logger.error(f"Ошибка при проверке заказа #{service_order_id}: {response.status_code}, {response.text}")
            delay = 300

    except requests.exceptions.RequestException as req_ex:
        logger.error(f"RequestException при проверке #{service_order_id}: {req_ex}")
        delay = min(300 * (2 ** (attempt - 1)), 3600)
    except Exception as ex:
        logger.error(f"Неизвестная ошибка при проверке заказа #{service_order_id}: {ex}")
        delay = 300

    logger.info(f"Повторная проверка заказа #{service_order_id} через {delay} сек.")
    threading.Timer(
        delay,
        check_order_status,
    ...
    if isinstance(e, NewMessageEvent):
        ...
        if m_check:
            ...
            try:
                ...
                c.send_message(msg_chat_id, f"❌ Ошибка при проверке статуса заказа")
            return

        # УДАЛЕН БЛОК РЕФИЛЛА (команда "рефилл")
...
def auto_smm_settings(message: types.Message):
    ...
    status_text = "✅ АКТИВИРОВАН" if RUNNING else "❌ ВЫКЛЮЧЕН"
...
Здесь вы найдете важные ссылки для работы с плагином и сервисами SMM.
    ...
    kb_.row(
        InlineKeyboardButton("🌐 OptSMM", url="https://optsmm.ru"),
...
• <b>Текущий баланс:</b> <code>{bal_:.2f} {currency_}</code>
...
def init_commands(c_: Cardinal):
    ...
    # Удален блок с реферальными ссылками на конкурента
    ...
    def add_website_prompt(call: types.CallbackQuery):
        ...
        msg_ = bot.edit_message_text("Введите домен для добавления (например: optsmm.ru):", call.message.chat.id,
                                     call.message.message_id)
    ...
    def process_add_website(message: types.Message):
        ...
        if not new_site:
            bot.send_message(message.chat.id, "❌ Нельзя добавить пустой домен.")
            return
            
        # Убираем http/https и пути
        new_site = new_site.replace('https://', '').replace('http://', '').split('/')[0]
        
        add_website(message, new_site)
        kb_ = InlineKeyboardMarkup()
        kb_.add(InlineKeyboardButton("🔙 Вернуться в настройки", callback_data="return_to_settings"))
        bot.send_message(message.chat.id, f"✅ Сайт {new_site} добавлен. Вернитесь в настройки для продолжения.", reply_markup=kb_)
    ...
    # Удалены все обработчики связанные с рефиллом
    ...
    def delete_all_lots_prompt(call: types.CallbackQuery):
        ...
        bot.edit_message_text("⚠️ Вы уверены, что хотите удалить ВСЕ лоты? Это действие нельзя отменить!", 
    ...
    def confirm_delete_all_lots(call: types.CallbackQuery):
        ...
        bot.edit_message_text("✅ Все лоты успешно удалены.", call.message.chat.id, call.message.message_id, reply_markup=kb)
    ...
    def lot_settings(call: types.CallbackQuery):
        ...
        kb.add(InlineKeyboardButton("🔙 Назад", callback_data="return_to_settings"))
        bot.edit_message_text("🛍️ Управление лотами:", call.message.chat.id, call.message.message_id, reply_markup=kb)

    @bot.callback_query_handler(func=lambda call: call.data == "show_lots_list")
    def show_lots_list(call: types.CallbackQuery):

        bot.edit_message_text("📋 Выберите лот для редактирования:", call.message.chat.id, call.message.message_id,
    ...
    def process_lot_search(message: types.Message):
        ...
        kb.add(InlineKeyboardButton("🔍 Новый поиск", callback_data="search_lot"))
        kb.add(InlineKeyboardButton("🔙 К настройкам лотов", callback_data="lot_settings"))
        bot.send_message(message.chat.id, f"🔍 Найдено {len(found_lots)} лотов по запросу '{search_term}':", reply_markup=kb)
...
⚫ За всё время:
   ...
   • Чистая прибыль: {stats['all_time_chistota']} ₽
            """.strip()

            kb = InlineKeyboardMarkup()
            kb.add(InlineKeyboardButton("🔄 Обновить", callback_data="show_profit_stats"))
   ...
   (чистая прибыль: {stats['all_time_chistota']} руб.)
        ...
        for f_ in files_to_send:
            if os.path.exists(f_):
                try:
                    ...
                    logger.error(f"Ошибка отправки {f_}: {e}")
            else:
                logger.warning(f"Файл не найден: {f_}")
                
        if sent_count > 0:
            bot.send_message(chat_id_, f"✅ Успешно отправлено {sent_count} файлов из {len(files_to_send)}")
        else:
            bot.send_message(chat_id_, "❌ Не удалось отправить ни одного файла")
        ...
        if os.path.exists(LOG_PATH):
            try:
                with open(LOG_PATH, 'rb') as f:
                    bot.send_document(chat_id_, f, caption="📝 Лог ошибок AutoSMM")
                bot.send_message(chat_id_, "✅ Логи ошибок выгружены.")
            except Exception as e:
                bot.send_message(chat_id_, f"❌ Ошибка отправки лог-файла: {e}")
        else:
            bot.send_message(chat_id_, "📝 Лог-файл не найден. Ошибок пока нет.")
    ...
    def delete_orders(call: types.CallbackQuery):
        ...
        try:
            ...
            # Создаем пустые файлы заново
            save_orders_data([])
            with open(ORDERS_PATH, 'w', encoding='utf-8') as f:
                json.dump([], f, indent=4, ensure_ascii=False)
                
            bot.edit_message_text("✅ Файлы заказов успешно удалены и очищены.", call.message.chat.id, call.message.message_id)
        except Exception as e:
            bot.edit_message_text(f"❌ Ошибка при удалении файлов: {e}", call.message.chat.id, call.message.message_id)
    ...
    def toggle_send_auto_lots(call: types.CallbackQuery):
        ...
        save_config(cfg)
        bot.answer_callback_query(call.id, f"📤 Отправка auto_lots.json: {'ВКЛ' if cfg['send_auto_lots'] else 'ВЫКЛ'}")
        misc_settings(call)
    ...
    def toggle_auto_start(call: types.CallbackQuery):
        ...
        save_config(cfg)
        bot.answer_callback_query(call.id, f"🚀 Автозапуск: {'ВКЛ' if cfg['auto_start'] else 'ВЫКЛ'}")
        misc_settings(call)
    ...
    def change_send_interval(call: types.CallbackQuery):
        ...
        kb_ = InlineKeyboardMarkup()
        kb_.add(InlineKeyboardButton("🔙 Вернуться назад", callback_data="misc_settings"))

        msg_ = bot.edit_message_text(
            f"⏱️ Текущий интервал отправки: {current_interval} минут\n\nВведите новый интервал отправки auto_lots.json в минутах (от 5 до 1440):",
            call.message.chat.id, call.message.message_id, reply_markup=kb_)
    ...
    def process_send_interval_change(message: types.Message):
        ...
        try:
            ...
            bot.send_message(message.chat.id, f"✅ Интервал отправки установлен: {new_interval} минут",
                             reply_markup=kb_)
        except ValueError:
    ...
    def change_name(call: types.CallbackQuery):
        ...
        lot_key = call.data.split("_", 2)[2]
        msg_ = bot.edit_message_text(f"✏️ Введите новое название для {lot_key}:", call.message.chat.id,
                                     call.message.message_id)
    ...
    def add_new_lot(call: types.CallbackQuery):
        ...
        msg_ = bot.send_message(call.message.chat.id, "🔢 Введите ID лота с FunPay для добавления:")
    ...
    def update_lot_ids(call: types.CallbackQuery):
        ...
        cfg = load_config()
        reindex_lots(cfg)
        bot.answer_callback_query(call.id, "✅ Номера лотов успешно обновлены.")
        misc_settings(call)
...
• <code>{{buyer_username}}</code> - имя пользователя
• <code>{{orderDesc}}</code> - описание заказа  
• <code>{{orderPrice}}</code> - цена заказа
• <code>{{orderAmount}}</code> - количество
...
• <code>{{link}}</code> - ссылка
    ...
    def process_add_service(message: types.Message):
        ...
        kb_ = InlineKeyboardMarkup()
        kb_.add(InlineKeyboardButton("🔙 К настройкам API", callback_data="api_settings"))
        bot.send_message(message.chat.id, f"✅ Сервис #{srv_num} добавлен. Не забудьте настроить URL и ключ!", reply_markup=kb_)
    ...
    def set_notification_chat_id(call: types.CallbackQuery):
        ...
        msg_ = bot.edit_message_text(
            f"💬 Введите Chat ID для уведомлений:\n\nВаш текущий ID: <code>{call.message.chat.id}</code>\n\nДля групп: -1001234567890",
...
def send_order_started_notification(c: Cardinal, order_id_funpay: str, service_order_id: int, link: str, api_url: str,
    ...
    try:
        ...
        notification_text = (
            ...
            f"💸 Стоимость услуги: {charge} {currency}\n"
...
def process_link_without_confirmation(c: Cardinal, data: Dict):
    ...
    try:
        ...
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Ваш API возвращает ЧИСЛО, а не объект
        if isinstance(j_, (int, float)):
            service_order_id = int(j_)  # Берем число напрямую
        elif isinstance(j_, dict) and "order" in j_:
            service_order_id = j_["order"]  # Для совместимости с другими API
        else:
            raise ValueError(f"Неожиданный формат ответа API: {j_}")
